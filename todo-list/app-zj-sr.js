...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('zjSr' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('zjSr' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosZjSr') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosZjSr'));
    }
    todos.push(todo);
    localStorage.setItem('todosZjSr', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('zjSr' + todoText);
