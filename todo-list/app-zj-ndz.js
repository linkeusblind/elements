...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('zjNdz' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('zjNdz' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosZjNdz') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosZjNdz'));
    }
    todos.push(todo);
    localStorage.setItem('todosZjNdz', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('zjNdz' + todoText);
