...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('zjWt' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('zjWt' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosZjWt') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosZjWt'));
    }
    todos.push(todo);
    localStorage.setItem('todosZjWt', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('zjWt' + todoText);
