...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('wt' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('wt' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosWt') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosWt'));
    }
    todos.push(todo);
    localStorage.setItem('todosWt', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('wt' + todoText);
