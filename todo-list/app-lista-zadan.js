...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('lzadCompleted' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('lzadCompleted' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosLzadan') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosLzadan'));
    }
    todos.push(todo);
    localStorage.setItem("todosLzadan", JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('lzadCompleted' + todoText);
