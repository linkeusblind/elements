...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('czw' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('czw' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosCzw') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosCzw'));
    }
    todos.push(todo);
    localStorage.setItem('todosCzw', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('czw' + todoText);
