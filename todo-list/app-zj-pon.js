...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('zjPon' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('zjPon' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosZjPon') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosZjPon'));
    }
    todos.push(todo);
    localStorage.setItem('todosZjPon', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('zjPon' + todoText);
