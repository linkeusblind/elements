...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('pt' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('pt' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosPt') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosPt'));
    }
    todos.push(todo);
    localStorage.setItem('todosPt', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('pt' + todoText);
