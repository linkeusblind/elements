...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('zjSob' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('zjSob' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosZjSob') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosZjSob'));
    }
    todos.push(todo);
    localStorage.setItem('todosZjSob', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('zjSob' + todoText);
