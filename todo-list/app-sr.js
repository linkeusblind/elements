...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('sr' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('sr' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosSr') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosSr'));
    }
    todos.push(todo);
    localStorage.setItem('todosSr', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('sr' + todoText);
