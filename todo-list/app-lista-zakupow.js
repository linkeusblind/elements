...
function deleteCheck(e) {
    ...
    if (item.classList[0] === 'complete-btn' || item.classList[0] === 'todo-item') {
        ...
        if (completed === true) {
            localStorage.setItem('lzakCompleted' + todoText, JSON.stringify(todoText));

        } else {
            localStorage.removeItem('lzakCompleted' + todoText, todoText);
        }
    }
...
function saveLocalTodos(todo) {
    let todos;
    if (localStorage.getItem('todosLzak') === null) {
        todos = [];
    } else {
        todos = JSON.parse(localStorage.getItem('todosLzak'));
    }
    todos.push(todo);
    localStorage.setItem('todosLzak', JSON.stringify(todos));
}
...
function getTodos() {
    ...
    todos.forEach(function (todo) {
        ...
        const localItem = localStorage.getItem('lzakCompleted' + todoText);
